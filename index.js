...
.setThumbnail()
.addField(`Provision | Teşekkürler`, `**Selamlar, Ben αlieи (Provision'un Geliştiricisi) Öncelikle Botumuzu Eklediğiniz ve Bize Destek Olduğunuz İçin Sizlere Teşekkürlerimi Sunarım**`)
.addField(`Provision | Prefix`, `**Provision Botun Prefixi(ön eki) = \`${fynx.prefix}\`\n\n Değiştirebilmek için \`${fynx.prefix}prefix\` Yazabilirsiniz.**`)
.addField(`Provision | Nasıl Kullanılır?`, `**Pirate botun tüm özelliklerinden yararlanabilmek için sadece \`${fynx.prefix}yardım\` yazmanız yeterlidir.**`)
.addField(`Provision | Linkler`, `**Sohbet Kanalına -davet Yazmanız Yeterlidir**`)
.setFooter(`Provision | Gelişmiş Türkçe Bot | 2020-2021`)
.setTimestamp();
...
var oyun = [
`✨ Yardım almak için | p!yardım`,
`🚀 Yeni Özellikler İçin | p!yardım`,
`🔔 Yenilenen Tasarımı İle`,
`⚡️ Botu eklemek için | p!davet`,
`🌟 Prefix ayarlamak için | p!prefix`
]
